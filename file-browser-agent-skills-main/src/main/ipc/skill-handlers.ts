...
import { installSkillFromZip, SKILLS_ROOT, SKILLS_STORE } from '../services/skills';
...
export function registerSkillHandlers(): void {
  ...
  ipcMain.handle('skill:list', () => SKILLS_STORE.listAvailable());
  ...
  ipcMain.handle('skill:add', async (e): Promise<AddSkillResult> => {
    ...
    try {
      ...
      await SKILLS_STORE.reload(); // refresh the in-memory catalog from disk
