...
import { SKILLS_STORE } from './skills';
...
export function makeTools(emit: Emit, client: Anthropic, workDir: string) {
  ...
  return [
    ...
    betaTool({
      name: 'activate_skill',
      description:
        'Load the full instructions for a relevant skill when a task matches one of the available Agent Skills. ' +
        'Call this tool before solving a task that clearly needs specialized instructions. ' +
        '== Available Agent Skills ==\n' +
        SKILLS_STORE.getCatalogText(),
      ...
      run: async ({ name }) => {
        ...
        if (!skill) {
          fail(id, `No skill named ${name}`);
          return `No skill named "${name}". Available skills: ${SKILLS_STORE.getBulletList()}.`;
        }
